...
SELECT sum(estoque) 
FROM farmacia 
WHERE categoria = 'blue' or categoria = 'balck';
...
SELECT categoria ,count(medicamento) 
FROM farmacia 
WHERE categoria 
is null GROUP BY categoria;
...
SELECT  concat(medicamento, '  ' , '(' , categoria, ')') as medicamento_e_categoria 
FROM farmacia 
...
SELECT  concat( id, ' - ' , medicamento, '(', coalesce(categoria, 'sem categoria'), ')') as medicamento_e_id FROM farmacia;
...
SELECT nome, idade, email, age(cast(cadastro as timestamp), now())
FROM usuarios
WHERE idade < 18 ;
...
SELECT idade, count(idade) as quantidade_por_idade
FROM usuarios
WHERE idade >= 18
GROUP BY idade;
